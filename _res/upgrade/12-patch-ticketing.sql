...
from ( 
	...
	from etracs255_cagban.sys_usergroup_member ugm 
		inner join etracs255_cagban.sys_usergroup ug on ug.objid = ugm.usergroup_objid 
	where ugm.usergroup_objid = 'TERMINAL.ADMIN' 
)t0, etracs255_cagban.sys_user u 
...
select 
	u.objid, u.username, u.lastname, u.firstname, u.middlename, u.name, u.jobtitle, u.txncode 
from ( 
	select distinct collector_objid as user_objid from etracs255_cagban.cashreceipt 
	union 
	select distinct subcollector_objid as user_objid from etracs255_cagban.cashreceipt 
)t0, etracs255_cagban.sys_user u 
where u.objid = t0.user_objid 
;
...
delete from etracs255_cagban.sys_usergroup_member where usergroup_objid like 'TERMINAL.%'
;
delete from etracs255_cagban.sys_usergroup where domain='TERMINAL'
;
...
select 
	objid, nextSeries 
from etracs255_cagban.sys_sequence 
...
from ( 
	...
	from etracs255_cagban.collectiontype ct
		inner join etracs255_cagban.collectiontype_org cto on cto.collectiontypeid = ct.objid 
